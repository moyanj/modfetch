...
- TOML 解析走 `adapters/config/toml_parser.py`（Python 3.11+ 用标准库 `tomllib`，3.10 用 `tomli`）；**勿改回 `toml` 库**（其数组要求同构，异构 `mods` 数组会解析失败）
- `ParentConfig.format` 校验仍列出 xml，但解析器未实现，文档按实际能力编写