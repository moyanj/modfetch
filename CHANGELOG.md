...
- **条件编译支持按加载器过滤（`only_loader`）**：条目新增 `only_loader`
  条件字段（支持字符串/列表），多加载器构建（`mod_loader = ["fabric", "forge"]`）
  时仅进入匹配加载器的产物；与 `only_version` / `feature` 组合为 AND 语义。
  贯穿计划生成（`plan_build`）、本地校验（含光影加载器关联校验升级为
  版本×加载器组合粒度）与远程校验（兼容性检查按组合过滤）；Web 配置编辑器
  同步支持编辑与展示。
...
- **lock 文件机制（可复现离线构建）**：新增 `lock` / `update` 子命令生成与更新 lock 文件
  （`update` 输出变更 diff）；`build --locked` 支持三态：lock 缺失时报错、指纹匹配时
  完全离线复用锁定计划、不匹配时自动重新解析，构建产物从此可复现。
- **新增 `search` 命令**：在 Modrinth 搜索模组，支持项目类型 / MC 版本 / 加载器过滤与
  `--limit` / `--json` 输出；搜索结果附带分类、支持的 MC 版本与更新时间等元信息。
- **新增 `add` 命令**：搜索后交互式选择将模组加入配置（`--yes` 免交互直取第一条，
  对齐 CI / 脚本场景）；TOML 增量写回保留注释与原有格式（YAML/JSON 全量重写），
  重复检测覆盖 `slug@version` 固定语法。
- **配置继承合并指令**：`$delete` / `$replace` / `$remove` / `$override` 四指令支持删除键、
  整体替换、按身份删除/覆盖列表项；继承解析新增循环引用检测（菱形继承不受影响），
  `file://` 相对路径基于配置文件所在目录逐级解析。

### 🐛 修复

- **Web 生命周期钩子弃用修复**：FastAPI 启动/关闭逻辑从弃用的 `on_event` 迁移至
  lifespan 异步上下文管理器，消除弃用告警。

### 📚 文档与测试

- **测试套件重构**：用例参数化并清理过时测试；新增 server / CLI / 下载 / 插件适配层
  单元测试、配置合并语义回归、lock 全流程（单元 + 集成）与 CLI 退出码契约测试。
- README 补充 `search` / `add` 与 lock 相关命令说明，并修正描述错误。
