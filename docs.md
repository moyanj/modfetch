...
> 需要更精细的继承控制时，可使用下方的合并指令。

#### **合并指令（高级语义）**

默认合并规则是"dict 递归、list 拼接去重、标量覆盖"，但有时需要更精细的控制——
比如去掉父配置里的某个模组、覆盖模组版本、完全替换列表、删除继承的键。
这时可以使用**合并指令**：将某个键的值设为带 `$` 前缀特殊键的 dict，
或向列表插入带 `$` 前缀键的 dict 元素。

##### 指令总览

| 指令 | 位置 | 语义 |
|---|---|---|
| `{ "$delete" = true }` | 作为某个键的值 | 从合并结果中删除该键 |
| `{ "$replace" = [...] }` | 作为某个键的值 | 整体替换该键的值（列表不再与父配置拼接） |
| `{ "$remove" = "slug" }` | 作为列表元素 | 按身份删除列表项（也支持值写成列表批量删除） |
| `{ "$override" = {...} }` | 作为列表元素 | 替换父配置中同身份的列表项（保持原位置），父配置无此项则追加 |

##### 规则说明

- 指令 dict 必须独占：不能与其他键混用
- `$remove` / `$override` 只能用于列表元素；`$delete` / `$replace` 只能作为键的值，混用位置会报错
- 身份匹配规则与去重一致：字符串 `"sodium"` 与 `{ id = "sodium" }` / `{ slug = "sodium" }` 视为同一身份
- `$remove` 对不存在的身份是静默 no-op；`$override` 对父配置中不存在的身份会追加到列表末尾
- 指令仅在使用 `from` 继承时有效；没有 `from` 的配置里出现指令会报错
- 指令按继承链逐层生效：每一层只影响它与上一层合并的结果
...
mods = [
    { "$remove" = "iris" },
    # 批量删除
    { "$remove" = ["phosphor", "entityculling"] },
]

# 覆盖父配置中 sodium 的版本（保持原位置）
mods = [
    { "$override" = { slug = "sodium", version = "0.6.0" } },
]

# 完全替换列表，不再继承父配置的 mods
mods = { "$replace" = ["sodium", "modmenu"] }

# 删除继承的键
[output]
download_dir = { "$delete" = true }
```

> **注意**：TOML 里 `$` 前缀键必须加引号（`"$remove"`），否则解析器会将其视为非法键名。
